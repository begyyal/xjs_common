...
mt.appendUnit("array2record", function (this: TestUnit<{
    class_a: CLS_A
}>) {
    this.chainContextGen(_ => ({ class_a: genCLS_A(1)[0] }));
    this.appendCase("basic functionality", function (this: TestCase, c) {
        const numary = [1, 2, 3];
        const o = UObj.array2record(numary, { vgen: n => n + "a" });
        this.check(numary.every(n => o[n] === n + "a"));
    });
    this.appendCase("pass values with kgen.", function (this: TestCase, c) {
        const numary = [1, 2, 3];
        const o = UObj.array2record(numary, { kgen: n => n + "a" });
        this.check(numary.every(n => o[n + "a"] === n));
    });
});
